...
def plot_linear_regression(slope, intercept, coordinates, ax = None):
    """
    Plots the linear regression on top of the coordinates
    """
    
    
    if ax is None:
        fig, ax = plt.subplots(figsize=(9, 7))
    xs = [el[0] for el in coordinates]
    ys = [el[1] for el in coordinates]
    ax.plot(xs,ys,'.', color = 'darkblue')

    y_preds = [slope*x + intercept for x in xs]
    ax.plot(xs,y_preds, color='orange')
